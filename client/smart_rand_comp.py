...
class Competitor:
    ...
    def play_turn(self, controller):
        ...
        total_comp_ammo = [controller.get_opponent_bot_ammo(i) for i in range(controller.NUM_BOTS) if i not in self.opp_dead]

        for bot in my_bots:
            possible_actions = ["LOAD", "LOAD", "LOAD"]
            for i in bot[AMMO]:
                possible_actions.append("LAUNCH")
            for i in range(total_comp_ammo):
                if i % 3 == 0:
